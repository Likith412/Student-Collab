   ...
   try {
      ...
      if (!username || !email || !password || !bio || !skills) {
         return res.status(400).json({
            message:
               "All fields are required: username, email, password, bio, and skills",
         });
      }
...
async function handleUpdateMyProfile(req, res) {
   try {
      ...
      const { username, bio, skills, githubLink, linkedinLink, emailLink } = req.body;

      // Validate required fields
      if (!username || !bio || !skills) {
         return res.status(400).json({
            message: "All fields are required: username, bio, and skills",
         });
      }
      ...
      // Optional links are removed when they are not sent back
      const fieldsToUnset = {};

      if (githubLink) {
         fieldsToSet.githubLink = githubLink.trim();
      } else {
         fieldsToUnset.githubLink = "";
      }
      if (linkedinLink) {
         fieldsToSet.linkedinLink = linkedinLink.trim();
      } else {
         fieldsToUnset.linkedinLink = "";
      }
      if (emailLink) {
         fieldsToSet.emailLink = emailLink.trim();
      } else {
         fieldsToUnset.emailLink = "";
      }
      ...
      if (updatedUser.githubLink) {
         userResponse.githubLink = updatedUser.githubLink;
      }
      if (updatedUser.linkedinLink) {
         userResponse.linkedinLink = updatedUser.linkedinLink;
      }
      if (updatedUser.emailLink) {
         userResponse.emailLink = updatedUser.emailLink;
      }
...
async function handleChangePassword(req, res) {
   try {
      ...
      // Validate that the new password is actually a new one
      if (currentPassword === newPassword) {
         return res.status(400).json({
...
async function handleUpdateUserBlockStatus(req, res) {
   try {
      const { userId } = req.params;
      ...
      // Admins are exempt from the block check, so blocking them has no effect
      if (user.role === "admin") {
      ...
      console.error("User block status update error:", error);
      return res.status(500).json({
         message: "Internal server error during user block status update",
      });
   }
}
