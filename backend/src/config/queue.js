...
const jobQueue = new Queue("jobs", REDIS_URL, { defaultJobOptions });

const handlers = {
   async sendEmail(payload) {
      const { deliverEmail } = require("../services/email.service");
...
function startQueueWorker() {
   jobQueue.process(QUEUE_CONCURRENCY, async job => {
...
async function enqueue(action, payload) {
   await jobQueue.add({ action, payload });
}
