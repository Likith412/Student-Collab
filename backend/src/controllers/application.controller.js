...
async function handleCreateApplication(req, res) {
   try {
      ...
      // Cancelled applications are ignored so a user can apply again after withdrawing
      ...
      if (project.status !== "open") {
         return res.status(400).json({
            message:
               "Cannot apply to this project. Project is not open for applications.",
         });
      }
...
async function handleUpdateApplicationStatus(req, res) {
   try {
      ...
      const isProjectOwner =
         application.projectId.createdBy.toString() === userId.toString();
      const isApplicant = application.userId.toString() === userId.toString();

      // Check Authorization
      if (!isProjectOwner && !isApplicant) {
      ...
      // Validate status
      // The project owner accepts or rejects, the applicant only withdraws
      const validStatuses = isProjectOwner
...
async function handleGetProjectApplications(req, res) {
   try {
      const { projectId } = req.params;
      ...
      if (project.createdBy.toString() !== userId.toString()) {
         return res.status(403).json({
            message: "You are not authorized to view the applications of this project",
         });
      }

      const applications = await Application.find({ projectId })
         .populate("userId", "username email bio skills")
         .sort({ createdAt: -1 })
         .lean();
